...
class SqlTemplate:
  ...
  def __init__(self, sql, regex=False, rename=True):  
    ...
    for item_idx, item in enumerate(self.parse):
      # only single line sql parses. No multiple statements
      # self.parse = [self.parse[0]]

      # Type the parse tree
      self.removeWhitespaces(self.parse[item_idx])
      self.identifyLiterals(self.parse[item_idx])
      self.parse[item_idx].ptype = SUBQUERY
      self.identifySubQueries(self.parse[item_idx])
      self.identifyFunctions(self.parse[item_idx])
      self.identifyTables(self.parse[item_idx])

      self.parseStrings(self.parse[item_idx])
      if rename:
        self.renameIdentifiers(self.parse[item_idx])
  ...
  def removeWhitespaces(self, tok):
    if isinstance(tok, sqlparse.sql.TokenList):
      tmpChildren = []
      for c in tok.tokens:
        if isinstance(c, sqlparse.sql.Token):
